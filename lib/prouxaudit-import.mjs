...
export const PROUX_IMPORT_VERSION = '2026-09-16.1';
...
export function normalizeProUXAuditPayload(payload, { sourceBase = 'https://prouxaudit.com', fetchedAt = new Date().toISOString() } = {}) {
  ...
  const normalized = payload.leads.map((lead) => normalizeLead(lead, sourceBase));
  if (new Set(normalized.map((lead) => lead.source_id)).size !== normalized.length) fail('duplicate_source_id_in_response');
  const leads = normalized.filter((lead) => lead.contact_state === 'available');
  return {
    leads, sourceBase, fetchedAt,
    coverage: { scope: normalized.some((lead) => lead.intent === 'result_followup') ? 'provided_export' : 'implementation_help_only', limit: PROUX_LEAD_LIMIT,
      returned: normalized.length, excluded_without_contact: normalized.length - leads.length, possibly_truncated: normalized.length === PROUX_LEAD_LIMIT,
