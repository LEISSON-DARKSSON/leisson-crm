...
// Kirjeldus EESTI KEELES, aga AINULT hanke enda oma.
//
// ULESANNE 12 NOUAB SEDA. segmentOf(title, kirjeldus) peab nagema MOLEMAT: ulesandes 3
// kaotas ainult pealkirja vaatav filter paris hanke (310983), sest RHR-i pealkiri on
// sageli asutuse sisenimi ja too sisu on kirjelduses. Lepinguteatel RSS-i kirjeldust
// ei ole, seega tuleb ta eForms-ist - vastasel korral oleks ajaloo segment SUSTEEMSELT
// kitsam kui elava hanke oma ja konkurentide pingerida naitaks meie nisist vale pilti.
//
// SKOOP ON NOUTUD: cac:ProcurementProject sees on ka cac:RealizedLocation/cbc:Description
// (tais- ja kohaaadress, moodetud fikstuuris 242 cbc:Description'it 11 teate kohta).
// Ilma skoobita satuks "Ida-Viru maakond, Sillamae linn, ..." hanke kirjelduse asemele.
// Loikame projekti esimese cac:-lapse eest ara - eForms-i elemendijarjekorras on
// cbc:Description ALATI enne cac:MainCommodityClassification'it.
function kirjeldusEst(pp) {
  if (typeof pp !== 'string') return null;
  const i = pp.indexOf('<cac:');
  const oma = i === -1 ? pp : pp.slice(0, i);
  const kirjeldused = koik(oma, 'cbc:Description');
  if (!kirjeldused.length) return null;
  return (kirjeldused.find((n) => attr(n.attrs, 'languageID') === 'EST') || kirjeldused[0]).sisu;
}
...
function loeNoticeResult(nr) {
  ...
  for (const p of plokid(nr, 'efac:TenderingParty')) {
    ...
    if (id && !pooled.has(id)) {
      // KONSORTSIUM. Uhel pakkumusel voib olla mitu pakkujat (moodetud fikstuuris:
      // TPA-0001 = ORG-0005 juhina + ORG-0004). Ulesanne 12 kirjutab MOLEMAD ridadena -
      // alltoovotupartner on tapselt see, keda konkurentide pingerida otsib - aga
      // efbc:GroupLeadIndicator utleb, kumb on juht, ja SUMMA laheb ainult juhi reale,
      // muidu loeks ulesande 13 mediaan sama lepingu raha kaks korda.
      pooled.set(id, plokid(p, 'efac:Tenderer').map((x) => ({
        id: uks(x, 'cbc:ID'),
        juht: /<efbc:GroupLeadIndicator(?:\s[^>]*)?>\s*true\s*</i.test(x),
      })).filter((x) => x.id));
    }
  }
...
// kolm osa, 10 / 7 / 7 pakkumust). Teate tasemel `tenders` on seetottu ESIMESE osa arv
// ja `osi` utleb, mitu osa tal uldse oli. OTSUS 21.09.2026: ulesanne 12 EI KIRJUTA
// enam uht rida teate kohta - ta loeb `lots`-massiivi, kus iga osa kannab OMA
// statistikat, tulemust ja voitjaid. Teate tasemel valjad jaavad alles pariteedivarava
// ja lihtsate loendurite jaoks, aga baasi granulaarsus on OSA.
...
// Osade oma projektikirjed: cac:ProcurementProjectLot/cbc:ID ('LOT-0001') ja selle
// sees OSA enda pealkiri, kirjeldus ja CPV. Teate tasemel ProcurementProject kirjeldab
// TERVIKUT; mitme osaga teatel on osa CPV sageli teine (paris naide: ehitus + hooldus).
// Ulesanne 12 kirjutab rea OSA kohta, seega ta vajab OSA valju, mitte ainult teate omi.
function loeOsad(keha) {
  const kaart = new Map();
  for (const p of plokid(keha, 'cac:ProcurementProjectLot')) {
    const id = uks(p, 'cbc:ID');
    if (!id || kaart.has(id)) continue;
    const pp = esimenePlokk(p, 'cac:ProcurementProject') || '';
    kaart.set(id, {
      title: nimiEst(pp),
      description: kirjeldusEst(pp),
...
const TUHI = {
  ref: null, notice_id: null, folder: null, date: null, title: null, description: null,
  ...
  winners: [], winner_arv: 0, tulemus: null, osi: null, lots: [],
...
export function parseAward(blk) {
  if (typeof blk !== 'string' || !blk) return { ...TUHI, winners: [], lots: [] };
  try {
    return loe(blk);
  } catch {
    return { ...TUHI, winners: [], lots: [] };
  }
}
...
function loe(blk) {
  ...
  // --- voitjad ja OSAD ---
  //
  // `lots` on ulesande 12 pohivaljund: TABELI GRANULAARSUS ON OSA, mitte teade.
  // Moodetud august 2026 (956 teadet): 54 teatel on nii mitu osa kui mitu voitjat ja
  // 17-l on osade tulemused erinevad. Uks rida teate kohta annaks sustemaatiliselt
  // vale konkurentide pingerea TAPSELT seal, kus raha on - suurtes mitmeosalistes
  // hangetes. Teate tasemel valjad (winner, tenders, amount) JAAVAD alles, sest
  // pariteedivarav ja hanke_sync loendurid toetuvad neile; nad kirjeldavad ESIMEST osa.
  const osaInfo = loeOsad(keha);
  let winners = [];
  ...
  if (nr !== null) {
    ...
    for (const lr of plokid(nr, 'efac:LotResult')) {
      // Osa tunnus tuleb efac:TenderLot viitest ('LOT-0001'); kui teda ei ole,
      // jaab LotResult-i oma ID ('RES-0000'). Uhtegi osa EI TOHI jaada ilma
      // tunnuseta - ilma selleta ei saa unikaalindeks osasid eristada.
      const lotId = uks(esimenePlokk(lr, 'efac:TenderLot') || '', 'cbc:ID') || uks(lr, 'cbc:ID');
      const lstat = loeStatistika(lr);
      const info = osaInfo.get(lotId) || null;
      const osa = {
        lot: lotId,
        tulemus: uks(lr, 'cbc:TenderResultCode'),
        tenders: lstat.tenders,
        tenders_allikas: lstat.allikas,
        title: info ? info.title : null,
        description: info ? info.description : null,
        cpv: info ? info.cpv : null,
        voitjad: [],
      };
      ...
      for (const tp of plokid(lr, 'efac:LotTender')) {
        ...
        const osalised = pooled.get(T.tp) || [];
        // Juht = see, kelle reale summa laheb. Margita pakkumusel (uks pakkuja voi
        // RHR ei margi juhti) on juht ESIMENE - mitte "ei kedagi", sest siis kaoks
        // summa kogu pakkumuselt ara.
        const juhtIdx = Math.max(0, osalised.findIndex((x) => x.juht));
        osalised.forEach((viide, i) => {
          const o = kaart.get(viide.id);
          if (!o) return;
          osa.voitjad.push({
            tender: tid,
            name: o.name ?? null,
            ...
            size: o.size ?? null,
            juht: i === juhtIdx,
            amount: i === juhtIdx ? T.amount : null,
            currency: i === juhtIdx ? T.currency : null,
          });
          if (nahtud.has(viide.id)) return;
          nahtud.add(viide.id);
          winners.push(o);
        });
      }
  ...
  return {
    ...
    title: nimiEst(pp),
    // Vt kirjeldusEst: ULESANNE 12 nisifilter vajab pealkirja KORVAL kirjeldust.
    description: kirjeldusEst(pp),
    ...
    // Ulesande 12 tabeli granulaarsus. Tuhi massiiv tahendab "sellel teatel ei ole
    // NoticeResult-i" - importija teeb siis teate tasemel UHE rea, et teade ei kaoks.
    lots,
    ...summa,
