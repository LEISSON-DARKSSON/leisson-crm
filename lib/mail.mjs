...
// Ainult lugemine: otsi kaustast (sent | inbox | drafts) kirju ImapFlow search-kriteeriumiga.
// Tagastab ümbrikud; kasutab järelkirja otsustaja (win/jarelkiri-mustand.mjs).
const KAUSTAD = {
  sent: [['\\Sent'], ['sent', 'saadetud']],
  drafts: [['\\Drafts'], ['drafts', 'mustandid', 'draft']],
};
export async function imapFind(accId, kind, criteria) {
  const acc = account(cfg(), accId);
  const c = await client(acc);
  try {
    const box = kind === 'inbox' ? 'INBOX' : await findBox(c, ...KAUSTAD[kind]);
    if (!box) return [];
    const lock = await c.getMailboxLock(box);
    try {
      ...
      for await (const m of c.fetch(uids, { envelope: true, internalDate: true }, { uid: true })) {
        out.push({ uid: m.uid, date: m.internalDate || m.envelope?.date, messageId: m.envelope?.messageId, subject: m.envelope?.subject });
      }
