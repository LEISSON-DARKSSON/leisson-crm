...
import {lisaLoobumisrida,sendGate,limits as saatmisLimiidid} from './sendgate.mjs';
...
// Saaja, kellele on JUBA kiri väljas — kas selle täpse kampaania kaudu, mõne
// TEISE kampaania kaudu või täiesti kampaaniaväliselt käsitsi (nt AS SA.MET
// 20.09.2026: kiri saadeti käsitsi kell 17:13, aga tema ootel-rida jäi
// varasemasse kinnitatud kampaaniasse rippuma) — ei tohi rippuda pending
// reana pooleliolevas kampaanias. runCampaignOnce blokeerib sellise rea
// REAKTIIVSELT alles siis, kui sweep sinnamaani jõuab; see funktsioon teeb
// sama asja ETTEVAATAVALT, iga kord kui kampaaniaid loetakse, nii et Gert ei
// näe kunagi eksitavat "ootel" rida saaja jaoks, kellele kiri on juba väljas.
// Tagastab äsja blokeeritud read (tühi, kui midagi polnud teha — idempotentne).
export function pruneAlreadySentItems(db) {
  const stale = db.prepare(`
    SELECT i.id, i.campaign_id, i.company_id, c.name FROM sales_campaign_items i
      JOIN sales_campaigns s ON s.id = i.campaign_id
      LEFT JOIN companies c ON c.id = i.company_id
     WHERE s.status IN ('prepared','approved') AND i.status = 'pending'
       AND (
         EXISTS(SELECT 1 FROM activity a WHERE a.company_id = i.company_id AND a.kind = 'sent')
         OR EXISTS(SELECT 1 FROM outbound_messages m WHERE m.company_id = i.company_id AND m.state = 'accepted')
       )`).all();
  if (stale.length) {
    const block = db.prepare("UPDATE sales_campaign_items SET status='blocked',error=? WHERE id=?");
    for (const row of stale)
      block.run('Saajale on juba kiri saadetud (kampaaniaväliselt või varasemast kampaaniast) — eemaldatud pakutavast kampaaniast', row.id);
  }
  return stale;
}
...
// Saajad, kes on PRAEGU 'pending' mõnes kinnitamata või kinnitatud
// kampaanias — need ei tohi sattuda uue kampaania valikusse (vt
// prepareCampaign'i kommentaar ummiku kohta). Ilma companyIds-ita
// tagastab kogu praeguse "juba järjekorras" hulga, mida kasutab
// campaigns.html valikuekraan candidaatide peitmiseks.
export function queuedCompanyIds(db, companyIds) {
  const where = `s.status IN ('prepared','approved') AND i.status = 'pending'`;
  if (!companyIds) {
    return db.prepare(
  ...
  return db.prepare(
    `SELECT DISTINCT i.company_id, c.name FROM sales_campaign_items i
       JOIN sales_campaigns s ON s.id = i.campaign_id
       LEFT JOIN companies c ON c.id = i.company_id
      WHERE ${where} AND i.company_id IN (${placeholders})`
  ).all(...companyIds);
}
...
export function prepareCampaign(db, companyIds, {accountId,accounts,composeText,composeHtml,now=new Date()}={}) {
  ...
  try {
    ...
    // (campaigns.html peidab need saajad valikust juba ette — vt
    // GET /api/campaigns queuedCompanyIds — see kontroll on viimane, mitte
    // ainus liin, sest UI nimekiri võib olla mõni sekund vana.)
    const overlap=queuedCompanyIds(db,companyIds);
...
export function nextApprovedCampaign(db) {
  // Teine sortimisvõti EI OLE ilustus. Kaks kampaaniat võivad saada täpselt
  // sama approved_at väärtuse (sama sekund; kinnitamine käib ISO-sõnena).
  // Ainult approved_at järgi sortides tuli ülejäänud järjekord
  // idx_campaign_queue kattva indeksi skaneerimisest, st campaign_id
  // (randomUUID) järgi — reegel "varem kinnitatu võidab" kehtis siis
  // mündiviskena (mõõdetud 21.09.2026: 6 valet valikut 12 katsest,
  // gate-campaign.mjs kukkus ~40% jooksudest). c.rowid on ridade
  // sisestusjärjekord, st kampaania loomise järjekord: stabiilne ja
  // tähenduslik tie-break nii testis kui tootmises.
  return db.prepare(`SELECT c.id FROM sales_campaigns c
    ...
    ORDER BY c.approved_at ASC, c.rowid ASC, i.position ASC LIMIT 1`).get()?.id || null;
}

export async function runCampaignOnce(db,id,{accountId,accounts,composeText,composeHtml,send,now=new Date(),limits}={}) {
  // Kinnitatud kampaania EI sõltu SEND_LISTS-ist. Listi-filter on sihtide
  // VALIKU tööriist (agent/saatja.mjs otsustab, keda üldse kõnetada); siin on
  // saajad juba inimese poolt külmutatud ja kinnitatud ümbrikuna. Kui see
  // filter jääks peale, langeks iga kinnitatud rida, mille firma on mõnes
  // muus listis, kohe ja JÄÄDAVALT `blocked`-iks (vt allpool). Kõik ülejäänud
  // värava kontrollid — summutus, vigane aadress, vabapost, lubatud päev,
  // tempo/päevalimiit/tööaeg — kehtivad muutumatult, sest postkast on üks.
  const gate=sendGate(db,{now,L:{...(limits||saatmisLimiidid()),lists:[]}});
