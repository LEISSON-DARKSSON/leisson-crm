...
export function approveCampaign(db,id,hash,{now=new Date(),actor='user:local-crm'}={}) {
  if(actor!=='user:local-crm')throw new Error('Kampaania kinnitamine on ainult omanikule');
  db.exec('BEGIN IMMEDIATE');
  try{
...
export async function runCampaignOnce(db,id,{accountId,accounts,composeText,composeHtml,send,now=new Date(),limits}={}) {
  ...
  if(!gate.eligibleIds.includes(next.company_id))
    return {status:'waiting',reason:'Saaja ei läbi tänast saatmisväravat'};
