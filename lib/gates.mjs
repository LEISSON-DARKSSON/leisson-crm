import {assertMailSource} from './mail-source-scope.mjs';
...
import { messageSelectionId, resolveMessageSelection } from './mail-identity.mjs';
...
function rows(db, ids) {
  ...
  for(const id of [...new Set(ids||[])]) {
    try { const m=assertMailSource(resolveMessageSelection(db,id));out.push({...m,id:messageSelectionId(m),requested_id:id,vanus:Math.floor((Date.now()-Date.parse(m.ts))/86400000)}); }
    catch(e){skip.push({id,why:e.message});}
  }
  return {out,skip};
}
...
export function bulkGate(db, task, ids) {
  ...
  const L = limits();
  const selected = rows(db, ids);
  const all = selected.out;
  const pass = [];
  const skip = [...selected.skip];

  for (const m of all) {
    if (m.deleted) { skip.push({ id: m.requested_id, why: 'kustutatud' }); continue; }
    if (spec.gate === 'mustand') {
      if(ownerSalesPause(db,{address:m.addr,companyId:m.company_id})){skip.push({id:m.requested_id,why:OWNER_SALES_PAUSE_REASON});continue;}
      const decision = replyDecision(m, { maxAgeDays: L.maxAgeDays });
      if (!decision.allowed) { skip.push({id:m.requested_id,why:decision.reason}); continue; }
      if (isMachine(m.addr)) { skip.push({ id: m.requested_id, why: 'masinaadress' }); continue; }
      if (m.vanus > L.maxAgeDays) { skip.push({ id: m.requested_id, why: `vanem kui ${L.maxAgeDays} päeva` }); continue; }
      if (!m.category || !VASTATAVAD.includes(m.category)) { skip.push({ id: m.requested_id, why: 'kategooria ei ole vastatav' }); continue; }
      if (m.review || m.suspicious) { skip.push({ id: m.requested_id, why: 'ülevaatuses või kahtlane' }); continue; }
      if (m.replied) { skip.push({ id: m.requested_id, why: 'juba vastatud' }); continue; }
      const d = db.prepare('SELECT status FROM drafts WHERE account=? AND uid=?').get(m.account, m.uid);
      if (d && d.status !== 'tagasi_lukatud') { skip.push({ id: m.requested_id, why: 'mustand on juba olemas' }); continue; }
    }
    pass.push(m.id);
  }
