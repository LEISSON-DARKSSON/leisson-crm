  ...
  for (const rida of String(tekst ?? '').split(/\r?\n/)) {
    ...
    if (pr) kaal = Number(pr[1]);
      ...
      if (lr && lr.index >= lopp && lr.index - lopp < KVAL_LAHEDUS) kaal = Number(lr[1]);
        ...
        // KOLMAS KUJU, MOODETUD 315437 pealt: real on osakaalu JAREL veel
        // ühikuveerg, seega number ei ole rea lõpus:
        //   „ 2  Proovitöö  ... Kvaliteet - hankija hinnatav   60      Punkt"
        // Ilma selleta jäi kvaliteedikaal NULL-i hankel, mille kvaliteedikaal on
        // 60 % ehk suurem kui hind. Ühikute nimekiri on KITSAS ja number peab
        // olema temast TUHIKUGA eraldatud — „100-VÄÄRTUSPUNKTI süsteemis"
        // (hange 312645, vale leid 100) jääb sidekriipsu tõttu välja.
        const yr = /(?<![\d,.])(\d{1,3})\s+(?:punkt|punkti|protsent|protsenti|ühik|ühikut|tk|eur)\b/iu.exec(rida);
        if (yr && yr.index >= lopp && yr.index - lopp < KVAL_LAHEDUS) kaal = Number(yr[1]);
      }
    }
...
// --- isikupõhine blokeeriv nõue --------------------------------------------

// MIKS SEE OLEMAS ON (mõõdetud 21.09.2026, hange 315437, TTJA rakendusuuring).
// Selle hanke AINUS tehnilise pädevuse tingimus oli:
//   „Vähemalt ühel võtmeeksperdil (nt andmeanalüütikul või andmeteaduse
...
// Käibenõuet ei olnud, ettevõtte referentsi ei olnud, rolle nõuti ÜKS. Ehk kõik
// senised väravad (`rollid >= 3`, `kaive > 50k`) vaikisid ja verdikt jäi KAALU,
// kuigi LEISSON ei kvalifitseeru sellele hankele ÜLDSE ilma partnerita. Lause
// ise jõudis leidude nimekirja — aga ainult märkega 'kontrolli' rolli
// „analüütik“ pealt, ehk otsust ta ei liigutanud.
//
// See värav on ISIKUPOHINE ja seetõttu teistsugune kui rollide arv: siin ei
// loeta, MITU inimest vaja on, vaid kas nõutav on KRAAD VOI KUTSE, mida majas
// lihtsalt EI OLE ja mida ei saa nädalaga tekitada.
//
// KITSAS TEADLIKULT. Muster nõuab kohustust SAMAS lauses ja välistab lause,
// mis räägib hindamisest („kõrgemalt hinnatakse doktorikraadi") — see on
// hindamiskriteerium, mitte kvalifitseerimistingimus, ja selle pealt ei tohi
// hanget käest anda.
export const BLOKEERIVAD = Object.freeze([
  ['doktorikraad', /doktorikraad\w*|doktorant(?:uur\w*|i\w*)?|doktoriõpp\w*|teaduskraad\w*|\bPhD\b/giu],
  ['kutsetunnistus', /kutsetunnistus\w*|kutsekvalifikatsioon\w*|kutsetase\w*/giu],
  ['atesteering', /atesteeri\w*|volitatud\s+insener\w*|pädevustunnistus\w*/giu],
  ['litsents', /tegevusluba\w*|litsents\w*(?!\s*tasu)/giu],
]);

// Hindamine EI OLE kvalifitseerimine.
const HINDAMISKEEL = /\bhinnatakse\b|\bhindamiskriteerium\w*|\bosakaal\w*|\blisapunkt\w*|\beeliseks\b/iu;
// SEADUSETSITAAT EI OLE SELLE HANKE NOUE. Mõõdetud 315437 hankepassist: lause
// „Viide seadusele: RHS § 101 lg 1 p 6 „andmed pakkuja ... hariduse ja
// KUTSEKVALIFIKATSIOONI kohta ..."" andis kindla nõude „kutsetunnistus", mida
// selles hankes ei nõutud. Siin oli ta kahjutu (doktorikraad blokeeris juba),
// aga just see viga ei anna endast kunagi märku — vale ALLTÖÖVÕTT tähendab
// hanget, mida me ei paku.
const TSITAAT = /viide\s+seadusele|RHS\s*§|\bseaduse\s+§|direktiiv\w*\s+\d|üldmääruse\s+artikl\w*/iu;

export function leiaBlokeeriv(tekst, fail = null) {
  const leiud = [];
  const kindlad = new Set();
  for (const lause of laused(tekst)) {
    for (const [nimi, muster] of BLOKEERIVAD) {
      muster.lastIndex = 0;
      if (!muster.test(lause)) continue;
      const kohustus = KOHUSTUS.test(lause);
      const hindamine = HINDAMISKEEL.test(lause);
      // SULUD LOIGATAKSE VABATAHTLIKKUSE TESTIST VALJA ja see tuli päris
      // lausest: „Vähemalt ühel võtmeeksperdil (NT andmeanalüütikul või
      // andmeteaduse eksperdil) PEAB OLEMA: doktorikraad ..." — „nt" käib seal
      // EKSPERDI NIMETUSE, mitte nõude kohta. Ilma selle lõiketa luges muster
      // 315437 kõige olulisema lause vabatahtlikuks ja verdikt jäi valeks.
      // Rollide juures õpitud vastupidine juhtum („õigus kaasata TÄIENDAVAID
      // spetsialiste (nt ...)") jääb endiselt kinni, sest tema vabatahtlikkus
      // on sulgudest VÄLJASPOOL.
      const ilmaSulge = lause.replace(/\([^)]{0,200}\)/gu, ' ');
      const tsitaat = TSITAAT.test(lause);
      const kindel = kohustus && !hindamine && !tsitaat && !VABATAHTLIK.test(ilmaSulge);
      if (kindel) kindlad.add(nimi);
      leiud.push({ liik: 'blokeeriv', noue: nimi, kindlus: kindel ? 'kindel' : 'kontrolli',
        lause: toend(lause), fail,
        markus: kindel ? null
          : hindamine ? 'nõue esineb hindamise kontekstis — see on kriteerium, mitte kvalifitseerimistingimus'
            : tsitaat ? 'lause on seadusetsitaat, mitte selle hanke tingimus'
              : 'nõudesõna puudub lauses — kontrolli käsitsi' });
    }
  }
  return { nouded: [...kindlad], leiud };
}
...
export function koguLeiud(failid) {
  ...
  for (const f of failid) {
    ...
    leiud.push(...q.leiud);
    const b = leiaBlokeeriv(tekst, nimi);
    for (const v of b.nouded) blokeerivad.add(v);
    leiud.push(...b.leiud);
  }
  ...
  return {
    ...
    // Blokeerivad nõuded LIIDETAKSE nagu rollid: sama nõue kahes failis on üks
    // nõue. Tühi massiiv tähendab „ei leidnud", mitte „ei ole" — tõend on leidude
    // nimekirjas ja märkega 'kontrolli' leid siia EI jõua.
    blokeerivad: [...blokeerivad],
