...
// CRM_DB_PATH on siin KÄSUPÕHINE, mitte kõigi käskude ühine (audit PR2 järelparandus,
// 22.09.2026): sync/history/docs avavad baasi ISE (avaBaas() -> lib/db.mjs open(),
// mis loeb dbPath = process.env.CRM_DB_PATH || DB_PATH) - ilma selleta avaksid nad
// vaikimisi data/crm.sqlite (TOOTMISBAAS), kui käivitaja ise dbPath-i ei anna.
// 'gate' (test/gate-hanked.mjs) EI kutsu kunagi avaBaas()-i ega open()-i ilma
// EXPLITSIITSE dbPath-ita (iga kutse selles failis annab oma fikstuuritee kaasa) -
// CRM_DB_PATH jõudmine tema keskkonda oli seega SURNUD pärand ilma tarbijata, mitte
// "ühine baasmuutuja". Sama loogika kehtib tundmatu/tuleviku käsu vaikeväärtusele
// (vt lubatudEnv): kui KASU_ENV-is puudub kirje, on turvalisem MITTE anda
// CRM_DB_PATH-i kui anda seda vaikimisi.
export const KASU_ENV = Object.freeze(Object.assign(Object.create(null), {
  sync: { lisa: ['CRM_DB_PATH', 'HANKED_RSS_URL'], runId: true },
  history: { lisa: ['CRM_DB_PATH', 'HANKED_AWARD_BASE'], runId: true },
  docs: { lisa: ['CRM_DB_PATH', 'HANKED_DOCS_DIR', 'HANKED_RHR_BASE', 'PDFTOTEXT'], runId: true },
  ...
  // enda tulemuse (audit PR2, ENV-5). EI SAA ka CRM_DB_PATH-i (vt kommentaar ülal) -
  // fail ei loe seda muutujat kunagi ise, iga tema enda avaBaas()/open() kutse annab
  // oma fikstuuritee otse argumendina kaasa.
...
const WIN_BASE = ['SystemRoot', 'WINDIR', 'COMSPEC', 'PATH', 'PATHEXT', 'TEMP', 'TMP',
  'LOCALAPPDATA', 'APPDATA'];

/**
 * Käsupõhine env-allowlist. EI anna edasi kogu process.env-i - ainult Windows'i
 * baasmuutujad ja käsu enda lisakonfiguratsioon (nt CRM_DB_PATH ainult sync/history/
 * docs-il - vt KASU_ENV kommentaar, gate ei loe seda muutujat kunagi ise).
 ...
 * Võtmekuju väljundis on alati kanooniline (nt 'PATH', mitte 'Path'). Konfliktireegel
 * (nt lähteallikas annab MÕLEMAD 'PATH' ja 'Path' korraga - Windows'i muutujanimed on
 * tõstutundetud, aga JS-objektis on need siiski kaks eri võtit): kanooniline kirjatüüp
 * (täpne 'PATH') VÕIDAB ALATI, sõltumata Object.keys(sourceEnv) kirjutusjärjekorrast -
 * see on kinnitatud lähtenõue, mitte "esimene vaste võidab" (audit PR2 järelparandus,
 * 22.09.2026: varasem versioon valis esimese Object.keys() vaste, mis oli
 * sisestusjärjekorra-sõltuv ja mittedeterministlik OS-üleselt). Alles siis, kui
 * kanoonilist kirjatüüpi lähteallikas EI PAKU, kasutatakse tõstutundetut tagavaraotsingut
 * (esimene sobiv võti Object.keys() järjekorras) - vt test/gate-hanked-env.mjs ENV-2.
 */
export function lubatudEnv(cmd, sourceEnv = process.env) {
  const kasu = KASU_ENV[cmd] || { lisa: [], runId: false };
  const votmed = Object.keys(sourceEnv);
  const out = {};
  for (const votikuju of [...WIN_BASE, ...kasu.lisa]) {
    let leitud;
    if (Object.hasOwn(sourceEnv, votikuju)) {
      leitud = votikuju;
    } else {
      for (const k of votmed) {
        if (k.toUpperCase() === votikuju.toUpperCase()) { leitud = k; break; }
      }
    }
...
export function startRun(db, cmd, args = {}, { spawnFn = spawn, bootId = BOOT_ID, root = ROOT } = {}) {
  ...
  try {
    ...
    const lapseEnv = (KASU_ENV[cmd] && KASU_ENV[cmd].runId)
      ? { ...baasEnv, HANKED_RUN_ID: String(id) }
      : baasEnv;
    laps = spawnFn(process.execPath, argv, { cwd: root, windowsHide: true, env: lapseEnv });
  } catch (e) {
...
// AUDIT PR1b (F3, 22.09.2026): madalama taseme protsessi-elususe kontroll, JAGATUD
// agent/hanked-sync.mjs-iga (import { elab as pidElab }). Vana versioon puudis IGA
// erindi surmana - EPERM (protsess on olemas, aga kuulub teisele kasutajale/ohuraamile)
// naitas siis vale "surnud" ja voinuks vabastada otsejooksu luku ajal, mil rida on
// tegelikult elus. Kolm juhtu, mitte kaks:
//   1. signaal 0 onnestub -> KINDLALT elav;
//   2. ESRCH ("No such process") -> KINDLALT surnud - AINUS koristamist lubav juht;
//   3. iga MUU viga (sh EPERM, aga ka tundmatu/ootamatu viga) -> EI TOHI vaikselt
//      tahendada "surnud": tagastame elav, sest me EI TEA, kas protsess on surnud.
export const elab = (pid) => {
  if (!pid) return false;
  try {
    process.kill(pid, 0);
    return true;
