...
function currentSource(db, value, now) {
  if(ownerSalesPause(db,{address:value.to,companyId:value.companyId}))throw new Error(OWNER_SALES_PAUSE_REASON);
  ...
  if(c&&ownerSalesPause(db,{address:c.email,companyId:c.id}))throw new Error(OWNER_SALES_PAUSE_REASON);
...
export async function dispatchOutbound(db, id, input, send, {now=new Date(),accounts,approvalActor='local-crm-operator'} = {}) {
  ...
  if(!['local-crm-operator','local-crm-approved-campaign'].includes(approvalActor))
    throw new Error('Kinnituse päritolu on vigane');
  ...
  try {
    ...
    db.prepare('UPDATE outbound_previews SET approved_at=?,approved_by=? WHERE id=?').run(now.toISOString(),approvalActor,id);
