...
export function reconcileSalesReplies(db, { now = new Date(), apply = false } = {}) {
  ...
  for (const m of db.prepare("SELECT * FROM messages WHERE direction='in' AND body_text IS NOT NULL ORDER BY ts ASC").all()) {
    ...
    const paused=ownerSalesPause(db,{address:m.addr,companyId:m.company_id});
    const d = paused ? {allowed:false,intent,reason:OWNER_SALES_PAUSE_REASON} : replyDecision({ ...m, reply_intent: intent }, { now });
  ...
  for (const companyId of companyIds) {
    ...
    if (!c) continue;
    if(ownerSalesPause(db,{address:m.addr,companyId}))continue; // A new reply cannot release an owner's pause.
