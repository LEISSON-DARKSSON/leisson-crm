...
export function open({ dbPath = process.env.CRM_DB_PATH || DB_PATH } = {}) {
...
export function seed(db, { seedDir = join(ROOT, 'seed') } = {}) {
  ...
  const files = ['parnu.json', 'plaan.json', 'parnu2.json'];
  let inserted = 0;
  ...
  const ins = db.prepare(`
    INSERT INTO companies (id,name,seg,loc,regcode,turnover,email,email_note,url,priority,offer,price,
                           finding,why,angle,meet_day,subject,body,lang,listid,status,updated)
