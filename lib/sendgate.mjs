...
// .env-i VARULUGEMINE. Uksi protsessi env-ist lugemine oli vaikne louks:
// ukski paris kutsuja (Task Scheduleri ulesanne, win\*.cmd, conductor'i
// spawnitud protsess) ei laadi .env-i protsessi keskkonnamuutujatesse, nii et
// .env-i kirjutatud SEND_LISTS oleks olnud moju ilma kirjata. Tapselt sama
// louks tabas 20.09.2026 AGENT_MODEL_PROVIDER-it agent/worker.mjs-is ja sai
// seal sama lahenduse. Jarjekord: protsessi env > .env fail > vaikevaartus.
let FAILI_ENV = null;
function failiEnv() {
  if (FAILI_ENV === null) { try { FAILI_ENV = rawEnv(); } catch { FAILI_ENV = {}; } }
  return FAILI_ENV;
}

/** Uks vaartus kolmest allikast. `sources` on olemas ainult testi jaoks. */
export function resolveEnv(key, fallback, { proc = process.env, file } = {}) {
  const fail = file || failiEnv();
  for (const src of [proc, fail]) {
    const v = src?.[key];
...
export function limits(sources) {
  const n = (k, d) => Number(resolveEnv(k, d, sources));
  return {
    perDay: n('SEND_PER_DAY', 40),        // kirju paevas kokku
    perRun: n('SEND_PER_RUN', 5),         // kirju uhe jooksuga
    perHour: n('SEND_PER_HOUR', 8),       // PUHANGUVARAV: kirju tunnis
    minGapMin: n('SEND_GAP_MIN', 7),      // minutit kahe kirja vahel
    hourFrom: n('SEND_HOUR_FROM', 9),     // tooaeg algus (kohalik)
    hourTo: n('SEND_HOUR_TO', 17),        // tooaeg lopp
    // Tuhi loend = ILMA listi-piiranguta. Seda kasutab ainult kinnitatud
    // kampaania: seal on saajad juba inimese poolt kulmutatud ja kinnitatud,
    // listi-filter on sihtide VALIKU tooriist (agent/saatja.mjs), mitte
    // valjasaatmise varav.
    lists: String(resolveEnv('SEND_LISTS', 'parnu,parnu2', sources))
...
export function sendGate(db, { now = new Date(), L = limits() } = {}) {
  ...
  // Tuhi L.lists = koik listid (kinnitatud kampaania). Muidu ainult nimetatud.
  const listid = Array.isArray(L.lists) ? L.lists.filter(Boolean) : [];
  const kohad = '?,'.repeat(listid.length).slice(0, -1);
  const jarjestus = 'ORDER BY CASE priority WHEN \'A\' THEN 0 WHEN \'B\' THEN 1 ELSE 2 END, name';
  const valjad = 'SELECT id, name, email, subject, body, listid, priority, meet_day, status FROM companies';
  const read = listid.length
    ? db.prepare(`${valjad} WHERE status='ootel' AND listid IN (${kohad}) ${jarjestus}`).all(...listid)
    : db.prepare(`${valjad} WHERE status='ootel' ${jarjestus}`).all();
