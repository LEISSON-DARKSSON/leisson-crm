...
export function migrateHanked(db) {
  ...
  // ISIKUPOHINE BLOKEERIV NOUE (315437, 21.09.2026): komadega eraldatud nimed,
  // nt 'doktorikraad'. Eraldi veerg ja mitte ainult `docs_leiud` sisse, sest
  // score() peab teda nägema ka hilisemal sünkimisel, täpselt nagu `rollid`.
  lisaVeerg(db, 'hanked', 'blokeeriv_noue', 'TEXT');
  ...
  // AUDIT PR1b (F1, 22.09.2026): lisaVeerg lisab last_good_* veerud NULL-ina -
  // vana(d) edukas rida, mis oli olemas ENNE seda migratsiooni (voi PR3-jargne
  // rida, kus veerud juba on, aga NULL), kaotaks oma toendatud lahtejoone. Backfill
  // on IDEMPOTENTNE (WHERE last_good_rows IS NULL) - kord taidetud rida ei puutu
  // teda enam kunagi, seega korduv migrateHanked ega katkestus ei riku midagi.
  // Kasutab rea ENDA vana ts-i, mitte migratsiooni praegust aega, ja EI fabritseeri
  // lahtejoont vigasest (ok=0) ega tundmatust katsest.
  // Loe-enne-kirjuta: SELECT ei taotle kirjutuslukku (WAL-lugeja), UPDATE
  // ainult siis, kui midagi PARISELT on teha. WHERE-tingimus jaab UPDATE-isse
  // MUUTUMATUNA - see ei asenda kirjutamise tingimust, ainult valdib tarbetut
  // luku-taotlust nulltoo juhul. Sobivaid ridu VOIB tekkida uuesti (nt
  // agent/hanked-history.mjs importMonthXml oma INSERT ei taida last_good_*-i) -
  // seega EI ehitata siia "korra tehtud, valmis igaveseks" lippu (audit PR2, p.6).
  const onSobivaid = db.prepare(`SELECT 1 FROM hanke_sync
      WHERE last_good_rows IS NULL AND ok = 1 AND rows > 0 LIMIT 1`).get();
  if (onSobivaid) {
    db.prepare(`UPDATE hanke_sync SET last_good_ts = ts, last_good_rows = rows
        WHERE last_good_rows IS NULL AND ok = 1 AND rows > 0`).run();
  }
  lepinguteVeerud(db);
...
export function dokumendiLeiud(hange) {
  ...
  try {
    ...
    return {
      ...
      pdftotextTee: j.pdftotextTee ?? null,
      ...
      blokeerivad: Array.isArray(j.blokeerivad) ? j.blokeerivad : [],
...
export function docsReast(rida) {
  ...
  // Blokeeriv nõue on TEKST, mitte arv — `pane` ei sobi. Tühi string ei ole nõue.
  const blok = String(rida.blokeeriv_noue ?? '').split(',').map((s) => s.trim()).filter(Boolean);
  if (blok.length) out.blokeerivad = blok;
...
export function score(h, { today = new Date().toISOString().slice(0, 10), ajalugu = null, docs = null } = {}) {
  ...
  // KOLMAS VARAV, MOODETUD hankel 315437 (TTJA rakendusuuring, 21.09.2026):
  // isikupõhine kvalifikatsiooninõue, mida majas ei ole (doktorikraad või
  // kehtiv doktorantuuri staatus). Seal olid rollid = 1 ja käibenõue puudus, ehk
  // mõlemad senised väravad vaikisid ja verdikt jäi KAALU — kuigi pakkumus
  // oleks lükatud tagasi enne sisulist hindamist. Nimekiri tuleb AINULT
  // üheselt mõistetavast mustrist (vt lib/hanked-leiud.mjs leiaBlokeeriv).
  const blokeerivad = Array.isArray(docs && docs.blokeerivad) ? docs.blokeerivad
    : Array.isArray(h.blokeerivad) ? h.blokeerivad : [];
  const blokeerivVarav = blokeerivad.length > 0;
  const allt = rolliVarav || kaiveVarav || blokeerivVarav;
  if (allt) {
    const pohjused = [];
    if (rolliVarav) pohjused.push(rollid + ' rolli CV-nõuet');
    if (kaiveVarav) pohjused.push('käibenõue ' + raha(kaive));
    if (blokeerivVarav) pohjused.push('isikupõhine nõue: ' + blokeerivad.join(', '));
