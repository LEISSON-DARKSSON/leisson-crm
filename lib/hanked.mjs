...
// liigutada: score() annab medianTenders >= 8 eest -10 ja verdikti piir on 35,
...
export function score(h, { today = new Date().toISOString().slice(0, 10), ajalugu = null, docs = null } = {}) {
  ...
  // ta liigutab hanke uhest otsusest teise UKSI. Kahe lepingu pealt arvutatud
  ...
  if (Number.isFinite(mediaanTenders)) {
    ...
    } else if (mediaanTenders >= 8) {
      points += add(-10, kirjeldus + ' ' + alus + ' lepingul');
    } else if (mediaanTenders <= 3) {
      points += add(5, kirjeldus + ' ' + alus + ' lepingul');
    } else {
      // 4...7 pakkujat jaab KAHE REEGLI VAHELE ja see on teadlik auk (sama
      // muster mis maksumuse vahemikus 140 001 - 1 000 000). Auk on NAHTAV:
      // ilma selle reata ei oleks paris jooksus (moodetud 21.09.2026, mediaan
      // 3.5 pakkujat 24 lepingul) score_why-s uhtegi marki sellest, et ajalugu
      // uldse vaadati - ja inimene ei saaks eristada "ajalugu ei liiguta
      // skoori" ja "ajalugu ei ole laetud" vahel.
      points += add(0, kirjeldus + ' ' + alus + ' lepingul — 4…7 jääb kahe reegli vahele');
    }
  }
