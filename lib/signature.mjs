// E-posti HTML-allkiri, genereeritud Orbit tokenitest (leisson-shared: orbit-tokens/orbit.tokens.json).
...
import { sharedFile } from './shared-path.mjs';
import { toLegacy } from '@leisson/shared/orbit-tokens/dtcg.mjs';
import { logoEmail } from './logo.mjs';

const TOKENS = sharedFile('orbit-tokens/orbit.tokens.json');

// @leisson/shared >= 2.0.0: kanoon on DTCG 2025.10 (objektväärtused). toLegacy annab CSS-stringid ja
// aktsepteerib ka v1 faili. Ära loe toorest JSON-it otse (test/gate-signature-tokens.mjs).
export function loadTokens() {
  return toLegacy(JSON.parse(readFileSync(TOKENS, 'utf8')));
}
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;max-width:460px;">
        <tr>
          <td style="padding:0 0 ${T.s2} 0;">
            ${logoEmail({ display: T.display, mono: T.data, ink: T.ink, ink3: T.ink3, ...(p.tagline === false ? { sub: '' } : {}) })}
          </td>
        </tr>
        <tr>
...
` : ''}              <tr>
  ...
  const text = `--\n${p.name}\n${p.role} · ${p.org}\n${p.legal}\n${[p.email, p.site, p.prices].filter(Boolean).join(' · ')}`;
